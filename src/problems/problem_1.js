...
const explanation = `The reason Celsius temperature is now a constant number is because we've set the celsius = -40, and did not update it accordingly. We can use the useState to achieve this.`;
...
export function Problem () {
    ...
    const [cel, setcelsius] = useState(0);
    var celsius = -40;
    ...
    return <>
        ...
        <hr />
        {fahrenheit} &deg;F is {cel.toFixed(2)} &deg;C
        <p>{explanation}</p>
    </>
}